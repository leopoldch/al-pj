...
class TestWebSocketIntegration(TransactionTestCase):
    ...
    async def test_connect_with_valid_token(self):
        ...
        with patch("core.websocket.consumers.get_async_redis_client", return_value=mock_redis):
            communicator = WebsocketCommunicator(self.application, f"/ws/?accessToken={token}")
            connected, _ = await communicator.connect()
    ...
    async def test_connect_invalid_token(self):
        communicator = WebsocketCommunicator(self.application, "/ws/?accessToken=invalid")
        connected, _ = await communicator.connect()
        self.assertFalse(connected)
        await communicator.disconnect()
