...
class TestAlbumView(unittest.TestCase):
    ...
    @patch("core.views.albums.AlbumService")
    @patch("core.views.albums.AlbumSerializer")
    def test_givenServiceReturnsAlbums_whenGet_thenShouldSerializeData(self, mock_serializer, mock_service):
        request = self.factory.get("/albums/")
        force_authenticate(request, user=self.mock_user)
        self.view.request = request
        self.view.format_kwarg = None
    ...
    @patch("core.views.albums.AlbumService")
    @patch("core.views.albums.AlbumSerializer")
    def test_givenAuthenticatedUser_whenGet_thenShouldReturn200(self, mock_serializer, mock_service):
        request = self.factory.get("/albums/")
        force_authenticate(request, user=self.mock_user)
        self.view.request = request
        self.view.format_kwarg = None
    ...
    @patch("core.views.albums.AlbumService")
    def test_givenValidData_whenPost_thenShouldCallServiceCreateAlbum(self, mock_service):
    ...
    @patch("core.views.albums.AlbumService")
    def test_givenValidData_whenPut_thenShouldCallServiceModifyAlbum(self, mock_service):
        ...
        mock_service.modifyAlbum.assert_called_once_with(TEST_ALBUM_ID, request.data, request.FILES)
