...
class TestProfileView(unittest.TestCase):
    ...
    def test_givenAuthenticatedUser_whenGet_thenShouldSerializeRequestUser(self, mock_serializer):
...
class TestPresenceIndicatorView(unittest.TestCase):
    ...
    def test_givenAuthenticatedUser_whenGet_thenShouldCallServiceGetPresenceData(self, mock_service):
    ...
    @patch("core.views.users.UserService")
    def test_givenServiceReturnsData_whenGet_thenShouldReturn200AndData(self, mock_service):
        request = self.factory.get("/presence/")
        request.user = self.mock_user
        force_authenticate(request, user=self.mock_user)
